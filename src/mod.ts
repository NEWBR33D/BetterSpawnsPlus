                                ...
                                for (const locations in this.locationArray) {
                                    ...
                                    if (this.config.betterSpawnsPlus.consoleLogs) {
                                        ...
                                        if (this.config.betterSpawnsPlus.locations[location].main.enabled && !this.config.betterSpawnsPlus.locations[location].spawnGenerator.enabled) {
                                            ...
                                            this.logger.log(`   > bloodhounds: ${countBloodhounds}`, this.logInfo);
                                        }
                                        ...
                                        if (!this.config.betterSpawnsPlus.locations[location].main.enabled && this.config.betterSpawnsPlus.locations[location].spawnGenerator.enabled) {
                                            ...
                                            this.logger.log(`   > bloodhounds: ${countAddBloodhound}`, this.logInfo);
                                        }
                                    ...
                                    const filePath = `${container.resolve<PreAkiModLoader>("PreAkiModLoader").getModPath(fileName)}info/consoleOutput/extended_logs_${location}.json`;
                                    
                                    fs.writeFileSync(filePath, '');
                                    fs.appendFileSync(filePath, JSON.stringify(initSpawns, null, 4));
                                }
